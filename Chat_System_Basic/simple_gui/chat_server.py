...
class Server:
    def __init__(self):
        ...
        self.indices={}
        self.chat_history = {}  # new: save per-user chat
        # sonnet
...
#==============================================================================
                ...
                # 📌 新增：记录进 chat_history
                self.chat_history[from_name].append({
                    "from": from_name,
                    "message": msg["message"]
                })
                for g in the_guys[1:]:
...
#==============================================================================
    ...
    def generate_image(prompt):
        ...
        filename = re.sub(r'\W+', '_', prompt)[:20] + ".png"
        path = os.path.join("generated_images", filename)
        os.makedirs("generated_images", exist_ok=True)
        with open(path, "wb") as f:
            f.write(b"fake image content")  # 未来换成 AI 图片
        return path

    def summarize_chat(history):
        if not history:
            return "[Summary] No chat history yet."
        text = " ".join([m["message"] for m in history[-20:]])
        return "[Summary] " + " ".join(text.split()[:30]) + "..."

    def extract_keywords(history, top_n=5):
        messages = [m["message"] for m in history[-50:]]
        if not messages:
            return "[Keywords] None"
        v = TfidfVectorizer(stop_words='english')
        X = v.fit_transform(messages)
        feature = v.get_feature_names_out()
        sums = X.sum(axis=0)
        keywords = [(feature[i], sums[0,i]) for i in range(len(feature))]
        keywords.sort(key=lambda x: x[1], reverse=True)
        return "[Keywords] " + ", ".join([k[0] for k in keywords[:top_n]])

    def analyze_sentiment(text):
        pol = TextBlob(text).sentiment.polarity
        if pol > 0.1: return "[Sentiment] 😊 Positive"
        if pol < -0.1: return "[Sentiment] 😡 Negative"
